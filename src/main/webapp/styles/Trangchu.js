...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (jsonData.length >= 2) {
        ...
        var loai = product1.loai;
        var url1 = product1.details_link;
        var imagePaths1 = [product1.image_path1].filter(function(path) {
            return path !== null;
        }).map(function(path) {
            if (path.startsWith('http://') || path.startsWith('https://')) {
                ...
                return '\n' +
                    'http://localhost:8080/Gradle___com_example___demo1_war/' + path;
            }
        });
        showTooltip1(loai, imagePaths1, 'tooltip' + product1.id, url1);

        var product = jsonData[0];
        var ghichu = product.ghichu;
        var url = product1.details_link;
        var imagePaths = [product.image_path1].filter(function(path) {
            return path !== null;
        }).map(function(path) {
            if (path.startsWith('http://') || path.startsWith('https://')) {
                return path;
        ...
        showTooltip(ghichu, imagePaths, 'tooltip' + product.id , url);
    } else {
...
function showTooltip1(content1, imageUrls1, tooltipId1, pageUrl) {
    ...
    var imageSizes = [
        { width: '', height: '200px' }, // kích thước của hình ảnh thứ nhất
    ...
    var imagePositions = [
        { left: '-85px', top: '-410px' }, // Vị trí của hình ảnh thứ nhất
...
function showTooltip(content, imageUrls, tooltipId, pageUrl) {
    ...
    var imageSizes = [
        { width: '', height: '300px' }, // kích thước của hình ảnh thứ nhất
        { width: '350px', height: '' }, // kích thước của hình ảnh thứ hai
        { width: '100px', height: '' },
        { width: '300px', height: '' },
        { width: '250px', height: '' },
    ...
    var imagePositions = [
        { left: '-42px', top: '-510px' }, // Vị trí của hình ảnh thứ nhất
        { left: '-245px', top: '-500px' }, // Vị trí của hình ảnh thứ hai
        { left: '170px', top: '-450px' },
        { left: '-20px', top: '-200px' },
        { left: '-230px', top: '-363px' },
