...
<%
    ...
    try {
        ...
        try (Connection conn = DriverManager.getConnection(url, dbuser, dbpassword)) {
            ...
            String sqlspnuoc = "SELECT * FROM productsnuoc ORDER BY id ASC";
            try (PreparedStatement pstmt = conn.prepareStatement(sqlspnuoc);
                ...
                while (rs.next()) {
                    ...
                    product.setImagePath("images/" + rs.getString("image_path"));
                    productsUnder50.add(product);
                }
            }

            // Query for products over 50
            String sqlspca = "SELECT * FROM productsca ORDER BY id ASC";
            try (PreparedStatement pstmt = conn.prepareStatement(sqlspca);
                 ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    Product product = new Product();
                    product.setId(rs.getInt("id"));
...
<div class="tocsp">
    <a href="#Nuoc">SP cải thiện nước</a><br/>
    <a href="#Ca">SP điều trị cá</a><br/>
</div>

<div class="main">
    <h2 class="DPS" >SP cải thiện nước</h2>
    <div class="nhanvat" id="Nuoc">
        <%
    ...
    <h2 class="DPS" >Sp điều trị cá</h2>
    <div class="nhanvat" id="Ca">
        ...
        %>
        <div class="char2" onclick="showTooltip1('<%= product.getPrice() %>',['<%= product.getImagePath() %>'], 'tooltip<%= product.getId() +49%>')">
            <p><b><%= product.getName() %></b></p>
...
<script>
    ...
    function redirectToDetailPage() {
        if (selectedProductId) {
            window.location.href = "Chitietspnuoc.jsp?id=" + encodeURIComponent(selectedProductId);
        } else {
    ...
    function redirectToDetailPage1() {
        if (selectedProductId1) {
            window.location.href = "Chitietspca.jsp?id=" + encodeURIComponent(selectedProductId1);
        } else {
            alert("Vui lòng chọn một sản phẩm để xem chi tiết.");
        }
    }
...
<%
    ...
    try (Connection conn = DriverManager.getConnection(url, dbuser, dbpassword);
        ...
        String sql1 = "SELECT * FROM productsnuoc WHERE id = 1";
        ...
        while (rs.next()) {
            ...
            jsonObject.put("image_path", "images/" + rs.getString("image_path"));
            jsonArray.put(jsonObject);
        }

        // Truy vấn dữ liệu từ bảng thongtinho
        String sql2 = "SELECT * FROM productsca WHERE id = 1";
        ...
        while (rs.next()) {
            ...
            jsonObject.put("name", rs.getString("name"));
            jsonObject.put("price1", rs.getDouble("price"));
            jsonObject.put("image_path", "images/" + rs.getString("image_path"));
            jsonArray.put(jsonObject);
        }
    } catch (Exception e) {
...
<script src="styles/Trangsp.js"></script>
